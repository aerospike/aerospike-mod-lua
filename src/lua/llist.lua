...
-- Last Update July 16,  2013: tjl
--
-- Keep this MOD value in sync with version above
local MOD = "llist_2013_07_16.m"; -- module name used for tracing.  
...
-- This is a stupid hack -- but until we can assign "nil" to a list entry,
-- we instead have to assign a "fake" entry to it.  Once we fix the server
-- list code (so that nil is a valid entry), we'll stop using this hack.
...
local function initializeLeaf(topRec, ldtList, leafRec, firstValue, pd, nd )
  ...
  if( ldtMap[R_StoreMode] == SM_LIST ) then
    ...
    info("[NOTICE!!!]<%s:%s>Not ready for BINARY MODE YET!!!!", MOD, meth );
    leafMap[LF_ListEntryTotal] = 0;
    leafMap[LF_ListEntryCount] = 0;
...
local function openSubrec( src, topRec, digestString )
  ...
  if( rec == nil ) then
    ...
    src.ItemCount = itemCount + 1;
    GP=F and print("\n000000000000000_Opening_SubRec_0000000000000000000000\n");
  ...
  if( F == true ) then
    local propMap = rec[SUBREC_PROP_BIN];
    showRecSummary( rec, propMap );
  end
...
local function objectCompare( ldtMap, searchKey, objectValue )
  ...
  if ( objectValue == nil ) then
    ...
    result = CR_ERROR;
  elseif( searchKey == nil ) then
    info("[INFO]<%s:%s> searchKey is nil:Free Pass", MOD, meth );
    result = CR_EQUAL;
  else
...
local function createAndInitESR( src, topRec, ldtList )
  ...
  GP=F and trace("[ENTER]: <%s:%s> LDT Summary(%s)",
    MOD, meth, ldtSummaryString(ldtList));
...
local function setLeafPagePointers( leafRec, leftDigest, rightDigest )
  ...
  GP=F and trace("[ENTER]<%s:%s> left(%s) right(%s)",
...
updateSearchPath(sp, propMap, ldtMap, nodeRec, position, keyCount)
  ...
  local rc = 0;
  GP=F and trace("[ENTER]<%s:%s> SP(%s) PMap(%s) LMap(%s) Pos(%d) KeyCnt(%d)",
  ...
  if( keyCount >= nodeMax ) then
    ...
    info("[HasRoom FALSE]<%s:%s>Level(%d) SP(%s)", MOD, meth, levelCount,
      tostring( sp ));
  else
    list.append( sp.HasRoom, true );
    info("[HasRoom TRUE ]<%s:%s>Level(%d) SP(%s)", MOD, meth, levelCount,
      tostring( sp ));
  end

  print("EXIT UPDATE SEARCH PATH");

  --GP=F and trace("[EXIT]<%s:%s> SP(%s)", MOD, meth, tostring(sp) );
  return rc;
...
local function scanLeaf(topRec, leafRec, startPosition, ldtMap, resultList,
  ...
  -- >>>>>>>>>>>>>>>>>>>>>>>>>  LIST  MODE <<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    -- next compare.
    local found = false;
    GP=F and trace("[LEAF SCAN]<%s:%s>Position(%d)", MOD, meth, startPosition);
    for i = startPosition, listSize, 1 do
      ...
      if( compareResult == CR_EQUAL ) then
        ...
        GP=F and trace("[Scan:FOUND]<%s:%s>Pos(%d) Key(%s) Object(%s)",
  ...
  local result_A = scanStatus;
  local result_B = ERR_OK; -- if we got this far, we're ok.

  GP=F and trace("[EXIT]<%s:%s> rc(%d) resultList(%s) A(%s) B(%s)", MOD, meth,
    rc, tostring(resultList), tostring(result_A), tostring(result_B));
...
treeSearch( src, topRec, sp, ldtList, searchKey )
  ...
  for i = 1, treeLevels, 1 do
    ...
    if( i < treeLevels ) then
      ...
      if( i < (treeLevels - 1) ) then
        ...
        GP=F and info("[DEBUG]<%s:%s> NEXT NODE: INNER NODE: Summary(%s)",
        ...
        GP=F and info("[DEBUG]<%s:%s> NEXT NODE: LEAF NODE: Summary(%s)",
      ...
      if( resultMap.Status == 0 ) then
        GP=F and info("[DEBUG]<%s:%s> LEAF Search Result:: Pos(%d) Cnt(%d)",
  ...
  GP=F and trace("[EXIT]<%s:%s>SearchKey(%s) ResultMap(%s) SearchPath(%s)",
      MOD,meth,tostring(searchKey),tostring(resultMap),tostring(sp));
...
local function populateLeaf( leafRec, objectList )
  ...
  local rc = 0;
  GP=F and trace("[ENTER]<%s:%s>ObjList(%s)",MOD,meth,tostring(objectList));
...
local function listInsert( myList, newValue, position )
  ...
  if( position > listSize ) then
    ...
    info("[MYLIST APPEND]<%s:%s> Appended item(%s) to list(%s)", MOD, meth,
    ...
    info("[MYLIST TRANSFER]<%s:%s> listSize(%d) position(%d)", MOD, meth,
      listSize, position );
...
local function leafInsert( topRec, leafRec, ldtMap, newKey, newValue, position)
  ...
  local rc = 0;
  GP=F and trace("[ENTER]<%s:%s> key(%s) value(%s) ldtMap(%s)",
    MOD, meth, tostring(newKey), tostring(newValue), tostring(ldtMap));

  info("[NOTICE!!]<%s:%s>Using LIST MODE ONLY - No Binary Support (yet)",
    MOD, meth );
...
local function getNodeSplitPosition( ldtMap, keyList, nodePosition, newKey )
  ...
  GP=F and trace("[ENTER]<%s:%s> ", MOD, meth );
  info("[NOTICE!!]<%s:%s> Using Rough Approximation", MOD, meth );
...
local function nodeInsert( keyList, digestList, key, digest, position )
  ...
  GP=F and trace("[ENTER]<%s:%s> KL(%s) DL(%s) key(%s) D(%s) P(%d)",
    MOD, meth, tostring(keyList), tostring(digestList), tostring(key),
    tostring(digest), position);

  if( position <= 0 ) then
    warn("[ERROR]<%s:%s> Need to Fix NodeInsert position(%d)",
      MOD, meth, position);
    error("[ERROR:LeafInsert] Search Path Position is wrong");
  end
...
local function  populateNode( nodeRec, keyList, digestList)
  ...
  local rc = 0;
  GP=F and trace("[ENTER]<%s:%s> keyList(%s) digestList(%s)",
    MOD, meth, tostring(keyList), tostring(digestList));

  local nodeItemCount = list.size( leftKeyList );
  local count = list.size( objectList );
...
local function splitRootInsert( src, topRec, sp, ldtList, key, digest )
  ...
  local rc = 0;
  GP=F and trace("[ENTER]<%s:%s> SRC(%s) SP(%s) LDT(%s) Key(%s) ",
  ...
  info("[STATUS]<%s:%s> About to Take and Drop::Key(%s) Digest(%s)",
    MOD, meth, tostring(keyList), tostring(digestList));
...
local function splitNodeInsert( src, topRec, sp, ldtList, key, digest, level )
  ...
  local rc = 0;
  GP=F and trace("[ENTER]<%s:%s> SRC(%s) SP(%s) LDT(%s) Key(%s) Lvl(%d)",
    MOD, meth, tostring(src), tostring(sp), tostring(key), tostring(digest),
    level );
  ...
  if( level == 1 ) then
    ...
    GP=F and print("\n\n <><Y><> !!! SPLIT INNER NODE !!! <><H><> \n\n");
    GP=F and print("\n\n <><I><> !!! SPLIT INNER NODE !!! <><E><> \n\n");
    GP=F and print("\n\n <><K><> !!! SPLIT INNER NODE !!! <><L><> \n\n");
    GP=F and print("\n\n <><E><> !!! SPLIT INNER NODE !!! <><P><> \n\n");
    GP=F and print("\n\n <><S><> !!! SPLIT INNER NODE !!! <><M><> \n\n");
    GP=F and print("\n\n <><!><> !!! SPLIT INNER NODE !!! <><E><> \n\n");
    ...
    local nodeRec = sp.RecList[nodeLevel];
...
-- ======================================================================
local function insertParentNode(src, topRec, sp, ldtList, key, digest, level)
  local meth = "insertParentNode()";
  local rc = 0;
  GP=F and trace("[ENTER]<%s:%s> SP(%s) LDT(%s) Key(%s) Dig(%s) L(%d)",
    MOD, meth, tostring(sp), ldtSummaryString(ldtList), tostring(key),
    tostring(digest), level );

  print("STARTING INTO INSERT PARENT POINTER \n");

  -- Extract the property map and control map from the ldt bin list.
  print("EXTRACT PROP MAP");
  local propMap = ldtList[1];
  print("EXTRACT LDT  MAP");
  local ldtMap  = ldtList[2];
  print("EXTRACT LDT  MAP");
  local binName = propMap[PM_BinName];
...
splitLeafInsert( src, topRec, sp, ldtList, newKey, newValue )
  ...
  info("[STATUS]<%s:%s> About to Take and Drop:: List(%s)", MOD, meth,
    tostring(objectList));
  ...
  info("\n[DEBUG]<%s:%s>: LeftList(%s) RightList(%s)", MOD, meth,
    tostring(leftList), tostring(rightList) );
...
-- ======================================================================
-- treeInsert( src, topRec, ldtList, newValue, stats )
-- ======================================================================
...
-- (*) newValue
-- (*) stats: bool: When true, we update stats
local function treeInsert( src, topRec, ldtList, newValue, stats )
...
--  MOD, meth, ldtSummaryString(ldtList), tostring(newValue), tostring(stats));
  ...
  if( ldtMap[R_TreeLevel] == 1 ) then
    ...
    firstTreeInsert( src, topRec, ldtList, newValue, stats );
  else
  ...
  if( rc == nil or rc == 0 ) then
    ...
    warn("[ERROR]<%s:%s>Insert Error::Ldt(%s) newValue(%s) stats(%s) rc(%s)",
    MOD, meth, ldtSummaryString(ldtList), tostring(newValue), tostring(stats),
    tostring(rc));
...
local function unTransformSimpleCompare(unTransform, dbValue, searchKey)
  ...
  if( searchKey == nil or searchKey == modDbValue ) then
...
unTransformComplexCompare(ldtMap, unTransform, dbValue, searchKey)
  ...
  if( searchKey == nil ) then
    ...
    compareResult = objectCompare( ldtMap, searchKey, modDbValue );
    if( compareResult == CR_EQUAL ) then
      resultValue = modDbValue;
    end
  end
...
local function scanList( resultList, ldtList, objList, searchKey, flag,
  ...
  -- If we don't have a searchKey, then we don't apply a searchKey, but
  -- pass evertything thru to filter.
...
-- ======================================================================
-- getNextLeaf( src, topRrec, leafRec  )
...
local function getNextLeaf( src, topRec, leafRec  )
  ...
  GP=F and trace("[ENTER]:<%s:%s> LeafSummary(%s)",
    MOD, meth, leafSummaryString(leafRec));
  ...
  if( nextLeafDigest ~= nil and nextLeafDigest ~= 0 ) then
    local nextLeafDigestString = tostring( nextLeafDigest );
...
-- ======================================================================
local function 
treeScan(src, resultList, topRec, sp, ldtList, searchKey, func, fargs )
  local meth = "treeScan()";
  local rc = 0;
  local scan_A = 0;
  local scan_B = 0;
  GP=F and trace("[ENTER]: <%s:%s> searchPath(%s) searchKey(%s)",
      MOD, meth, tostring(sp), tostring(searchKey) );
  ...
  while not done do
    ...
    if( scan_A == SCAN_CONTINUE ) then
      ...
      leafRec = getNextLeaf( src, topRrec, leafRec );
      if( leafRec == nil ) then
        done = true;
      end
    else
  ...
  GP=F and trace("[EXIT]<%s:%s>SearchKey(%s) ResultList(%s) SearchPath(%s)",
      MOD,meth,tostring(searchKey),tostring(resultList),tostring(sp));
...
local function localLListSearch( topRec, ldtBinName, searchKey, func, fargs )
  ...
  if( ldtMap[R_StoreState] == SS_COMPACT ) then 
    ...
    local sp = createSearchPath(ldtMap);
    rc = treeSearch( src, topRec, sp, ldtList, searchKey );
    if( rc == ST_FOUND ) then
      rc = treeScan( src, resultList, topRec, sp, ldtList, searchKey,
                    func, fargs );
    end
  end -- tree search
...
function llist_scan( topRec, ldtBinName, func, fargs )
  local meth = "listScan()";
...
function llist_delete( topRec, binName, key )
  ...
  GP=F and trace("[ENTER]<%s:%s>ldtBinName(%s) key(%s)",
  ...
  if( ldtMap[R_StoreState] == SS_COMPACT ) then 
    local compactList = ldtMap[R_CompactList];
    GP=F and info("[NOTICE]<%s:%s> Using COMPACT DELETE", MOD, meth);
    rc = scanList( nil, ldtList, compactList, key, FV_DELETE, nil, nil);
  else
    GP=F and info("[NOTICE]<%s:%s> Using >>>  TREE DELETE  <<<", MOD, meth);
