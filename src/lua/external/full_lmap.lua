...
localLMapInsert( topRec, ldtBinName, newName, newValue, createSpec )
  ...
  -- it's time to rehash our single list into the real subrecord organization.
  -- After the rehash (conversion), we'll drop into the regular LMAP insert.
  local totalCount = ldtMap[M_TotalCount];
  -- NOTE: Need to REDO the rehash method mentioned below to read the
  -- compact list and call subRecInsert().
  --
  -- If not
  ...
  if (totalCount + 1) >= ldtMap[M_Threshold] then
    -- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! TODO : FINISH THIS
    -- !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    warn("[ERROR]<%s:%s>REHASH is not yet working", MOD, meth );
    error( ldte.ERR_INTERNAL );
    convertCompactToSubRec(topRec, ldtCtrl )
  end

  -- In the case of LMAP, we call localInsert only if it is SS_COMPACT mode
  -- insertion of elements into the first LMAP bin like an lset-insert.
